...
- Workers MVP では標準 WebCrypto で安定運用しやすい PBKDF2-HMAC-SHA-256 を採用する。ハッシュ文字列にはアルゴリズム、反復回数、salt、hash を含め、将来の work factor 引き上げを検知できる形式にする。
...
- MVP の API セッションは `threadpick_session` Cookie にランダムトークンを保存し、D1 の `sessions.token_hash` には SHA-256 ハッシュのみを保存する。
